...
// $Id: METNtupler.cc,v 1.1 2010/04/18 21:44:55 fblekman Exp $
//
//
...
private:
  ...
  pfstuff pfccontainer_;
      ...
      Float_t eleD0[10];
      Float_t eleD0Sig[10];
      Float_t eleHcalIso[10];
      Float_t eleTrkIso[10];
      Float_t eleEcalIso[10];
  ...
  usePAT_(iConfig.getUntrackedParameter<bool>("usePAT",false))
{
  ...
  tree_->Branch("numJet",&numJet,"numJet/F");
  ...
  if(usePAT_){
     tree_->Branch("numEle",&numEle,"numEle/F");
  ...
  if(usePAT_){
     ...
     if(!usePF_){
        ...
        for(pfc=pfcands->begin(); pfc!=pfcands->end(); ++pfc){
           ...
           if(pfc->particleId()!=0 && abs(pfc->particleId())<=3){ // tracking based pf candidates:
